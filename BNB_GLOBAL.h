...
public:
	...
	void load_graph(int rid_n, ui* rid, vector< pair<int, int> >& vp, ui* ids)
	{
		n = rid_n;
		m = vp.size() * 2;
		pstart = new int[n + 1];
	...
	bool bb_s_truss_bar(int curS, clock_t start, bool& timeflag)
	{
		//pruning rules
		...
		if (del.size()) {
			...
			bool ret = bb_s_truss_bar(curS - del.size(), start, timeflag);
			//printf("%d\n", curS - todel.size());
		...
		if (max_eid != -1 && inS_e[max_eid] && notadj_e[max_eid] >= S) {
			...
			if (todel.size()) {
				...
				bool ret = bb_s_truss_bar(curS - todel.size(), start, timeflag);
				//printf("%d\n", curS - todel.size());
  ...
  //       cal_notadj(max_eid);
		...
		if (inS_e[min_eid])
		{
			...
			for (int i = 0; i < canselect && i < branch.size() && !ret; i++)
			{
				...
				ret |= bb_s_truss_bar(curS - 1, start, timeflag);
				addtoD_vertex(__LINE__, branch[i]);
			}
			...
			if (branch.size() >= canselect)
			{
				if (canselect == 0 || canadd(branch[canselect - 1])) {
					...
					ret |= bb_s_truss_bar(curS - branch.size() + canselect, start, timeflag);
			...
			else
			{
				if (canselect == 0 || canadd(branch[branch.size() - 1])) {
					...
					ret |= bb_s_truss_bar(curS, start, timeflag);
		...
		else
		{
			...
			if ((inS_v[u] && !inS_v[v]) || (inS_v[v] && !inS_v[u]))
			{
				...
				bool ret = bb_s_truss_bar(curS - 1, start, timeflag);
			...
			else if (!inS_v[u] && !inS_v[v])
			{
				...
				bool ret = bb_s_truss_bar(curS - 2, start, timeflag);
