...
public:
	...
	void load_graph_IE(int must_include_, int rid_n, ui* rid, vector< pair<int, int> >& vp, ui* old_id)
	{
		n = rid_n;
		m = vp.size() * 2;
		pstart = new int[n + 1];
	...
	bool find_bounded_stb(clock_t start, bool& timeflag)
	{
...
private:
	...
	bool enumeration_random(int v, bool& flag, clock_t start, bool& timeflag)
	{
		if (flag) return true;
		...
		if (!deleted_vertex[current_node])
		{
			...
			ret |= enumeration_random(v + 1, flag, start, timeflag);
	...
	bool enumeration(bool& flag, clock_t start, bool &timeflag) 
	{
		if (flag) return true;
