...
public:
	...
	void load_graph(int must_include_, int rid_n, ui* rid, vector< pair<int, int> >& vp, ui* ids, int c, ui* odID, vector<vector<int>> topc_ans)
	{
		...
		{
			//printf("%d %d\n", rid[ids[i]], odID[rid[ids[i]]]);
			oldID[rid[ids[i]]] = odID[rid[ids[i]]];
		}
	...
	bool bb_s_truss_bar(int curS, clock_t start, bool& timeflag, int& C)
	{
		//pruning rules
		//printf("u = %d v = %d i = %d and tri_cnt[i] = %d ", edge_list[2 << 1], edge_list[(2 << 1) + 1], i, tri_cnt[i]);
		if (svex.size() >= LB)
		{
		...
		if (del.size()) {
			...
			bool ret = bb_s_truss_bar(curS - del.size(), start, timeflag, C);
			//printf("%d\n", curS - todel.size());
		...
		if (max_eid != -1 && inS_e[max_eid] && notadj_e[max_eid] >= S) {
			...
			if (todel.size()) {
				...
				bool ret = bb_s_truss_bar(curS - todel.size(), start, timeflag, C);
				//printf("%d\n", curS - todel.size());
  ...
  //       cal_notadj(max_eid);
		...
		if (inS_e[min_eid])
		{
			...
			for (int i = 0; i < canselect && i < branch.size() && !ret; i++)
			{
				...
				ret |= bb_s_truss_bar(curS - 1, start, timeflag, C);
				addtoD_vertex(__LINE__, branch[i]);
			}
			...
			if (branch.size() >= canselect)
			{
				if (canselect == 0 || canadd(branch[canselect - 1])) {
					...
					ret |= bb_s_truss_bar(curS - branch.size() + canselect, start, timeflag, C);
			...
			else
			{
				if (canselect == 0 || canadd(branch[branch.size() - 1])) {
					...
					ret |= bb_s_truss_bar(curS, start, timeflag, C);
		...
		else
		{
			...
			if ((inS_v[u] && !inS_v[v]) || (inS_v[v] && !inS_v[u]))
			{
				...
				bool ret = bb_s_truss_bar(curS - 1, start, timeflag, C);
			...
			else if (!inS_v[u] && !inS_v[v])
			{
				...
				bool ret = bb_s_truss_bar(curS - 2, start, timeflag, C);
	...
	bool bb_s_truss_bar_iterall(int curS, clock_t start, bool& timeflag, int& size)
	{
		//pruning rules
		//printf("u = %d v = %d i = %d and tri_cnt[i] = %d ", edge_list[2 << 1], edge_list[(2 << 1) + 1], i, tri_cnt[i]);
		if (svex.size() >= LB)
		...
		if (del.size()) {
			...
			bool ret = bb_s_truss_bar_iterall(curS - del.size(), start, timeflag, size);
			//printf("%d\n", curS - todel.size());
			for (int x = del.size() - 1; x >= 0; x--)
			{
				addtoD_vertex(__LINE__, del[x]);
		...
		if (max_eid != -1 && inS_e[max_eid] && notadj_e[max_eid] >= S) {
			...
			if (todel.size()) {
				...
				bool ret = bb_s_truss_bar_iterall(curS - todel.size(), start, timeflag, size);
				//printf("%d\n", curS - todel.size());
				for (int x = todel.size() - 1; x >= 0; x--)
				{
					addtoD_vertex(__LINE__, todel[x]);
  ...
  //       cal_notadj(max_eid);
		...
		if (inS_e[min_eid])
		{
			...
			for (int i = 0; i < canselect && i < branch.size() && !ret; i++)
			{
				...
				ret |= bb_s_truss_bar_iterall(curS - 1, start, timeflag, size);
				addtoD_vertex(__LINE__, branch[i]);
			}
			...
			if (branch.size() >= canselect)
			{
				if (canselect == 0 || canadd(branch[canselect - 1])) {
					if (canselect) addtoS(branch[canselect - 1]);
					ret |= bb_s_truss_bar_iterall(curS - branch.size() + canselect, start, timeflag, size);
					if (canselect) delfrS(branch[canselect - 1]);
				}
			}
			else
			{
				if (canselect == 0 || canadd(branch[branch.size() - 1])) {
					if (canselect) addtoS(branch[branch.size() - 1]);
					ret |= bb_s_truss_bar_iterall(curS, start, timeflag, size);
					if (canselect) delfrS(branch[branch.size() - 1]);
				}
			}
			for (int i = pos; i >= 0; --i)
		...
		else
		{
			...
			if ((inS_v[u] && !inS_v[v]) || (inS_v[v] && !inS_v[u]))
			{
				...
				// printf("%d %d\n", min_eid, deleted_edges[min_eid]);
				delfrD_vertex(__LINE__, v);
				bool ret = bb_s_truss_bar_iterall(curS - 1, start, timeflag, size);
				addtoD_vertex(__LINE__, v);
			...
			else if (!inS_v[u] && !inS_v[v])
			{
				...
				delfrD_vertex(__LINE__, v);
				delfrD_vertex(__LINE__, u);
				bool ret = bb_s_truss_bar_iterall(curS - 2, start, timeflag, size);
				addtoD_vertex(__LINE__, u);
				addtoD_vertex(__LINE__, v);
				if (ret) return true;
