...
public:
	...
	void test()
	{
		...
		for (int i = 0; i < (m >> 1); i++)
		{
			printf("eid = %d, %d-%d\n", i, edge_list[i << 1], edge_list[(i << 1) + 1]);
		}
		delfrD_edge(0);
		delfrD_vertex(__LINE__, 0);
		delfrD_vertex(__LINE__, 2);

		for (int i = 0; i < n; i++)
			printf("%d\n", degree[i]);
		printf("\n");
		addtoD_vertex(__LINE__, 2);
		addtoD_vertex(__LINE__, 0);
		for (int i = 0; i < n; i++)
			printf("%d\n", degree[i]);
	}
	...
	bool bb_s_truss_bar(int curS, clock_t start, bool& timeflag)
	{
		...
		if (min_eid_s != -1 && min_tri_s + 2 + S < LB) return false; //!!!!!!!!!!!!!upper bound
